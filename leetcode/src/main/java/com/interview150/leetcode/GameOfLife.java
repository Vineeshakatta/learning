...
public class GameOfLife {
	...
	public static void main(String[] args) {
		...
		int[][] board = { { 0, 1, 0 }, { 0, 0, 1 }, { 1, 1, 1 }, { 0, 1, 0 } };
		...
		for (int r = 0; r < rows; r++) {
			for (int c = 0; c < cols; c++) {
				int liveNeighbors = neighbor(board, r - 1, c - 1) + neighbor(board, r - 1, c)
						+ neighbor(board, r - 1, c + 1) + neighbor(board, r, c + 1) + neighbor(board, r + 1, c + 1)
						+ neighbor(board, r + 1, c) + neighbor(board, r + 1, c - 1) + neighbor(board, r, c - 1);
